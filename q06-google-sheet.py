...
import pandas as pd
import json
import utility06 as ut
import database06 as db

from datetime import datetime

# Page configuration
st.set_page_config(page_title="ระบบจัดการร้านกระถางต้นไม้",page_icon="💚",layout="wide")
...
def show_dialogAddProd():
    ...
    brand = st.selectbox("แบรนด์", ["๋Joobs' Pottery", "Genevieve Green", "KraThingThing", "Jay's Tree"])
...
def show_dialogEditProd(row_idx,product):
    ...
    if product:
        with st.form(key=f"edit_form_{product['id']}"):
            ...
            edit_brand = st.selectbox("แบรนด์", ["๋Joobs' Pottery", "Genevieve Green", "KraThingThing", "Jay's Tree"].index(product["brand"]))
